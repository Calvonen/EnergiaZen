 ...
 *
 * Safety decisions use a conservative value (remaining energy minus current
 * uncertainty), so model/sensor disagreement can never make the reserve look
 * safer than the evidence supports. Invalid authoritative state fails closed
 * and produces no control decision.
 */
export function evaluateEnergyReserve(
  state: AuthoritativeEnergyState,
