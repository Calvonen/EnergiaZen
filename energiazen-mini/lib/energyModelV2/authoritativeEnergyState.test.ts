...
function assertClose(actual: number, expected: number, message: string) { if (Math.abs(actual - expected) > 0.000001) throw new Error(`${message}: expected ${expected}, got ${actual}`); }
export function runAuthoritativeEnergyStateUnitTests() {
  ...
  assertClose(reconciled.uncertaintyKwh, observed.uncertainty.energyKwh, "known heater energy is not reclassified as balance uncertainty");
  const degraded = createAuthoritativeEnergyState({ ledger: { ...baseLedger, modeledStoredEnergyKwh: baseLedger.observedStoredEnergyKwh + 2, sensorCorrectionGapKwh: 2 }, observedState: observed });
  assert(degraded.quality === "degraded", "larger model/sensor gap degrades diagnostics");
  assertClose(degraded.uncertaintyKwh, observed.uncertainty.energyKwh, "diagnostic sensor gap is separate from physical balance uncertainty");
  ...
  assertClose(largeLag.uncertaintyKwh, observed.uncertainty.energyKwh, "large sensor lag still does not erase known physical energy");
}
