create table if not exists public.electricity_prices (
  ...
  constraint electricity_prices_time_order check (end_time > start_time),
  constraint electricity_prices_one_price_per_hour unique (region, start_time)
);

create index if not exists electricity_prices_region_price_date_idx
  on public.electricity_prices (region, price_date, start_time);
