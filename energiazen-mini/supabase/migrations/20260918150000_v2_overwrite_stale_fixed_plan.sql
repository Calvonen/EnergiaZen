-- When automatic mode is active, V2 is the authoritative producer for both
-- today's and tomorrow's production rows. A stale fixed row from the previous
-- mode must not block the mirror upsert.
...
begin
  ...
  insert into public.heating_plans(
    plan_date, planned_hours, target_hours, mode, reason, timezone, updated_at
  )
  values (
    new.plan_date,
    mirrored_hours,
  ...
  if production_mode is distinct from 'automatic'
    ...
    raise exception
      using
        ...
        message = 'V2 production plan mirror did not become authoritative in automatic mode';
  end if;
