-- Route healthy no-change validation through the same transactional snapshot
-- checks as changed-plan publication, and remove obsolete unsafe overloads.
...
begin
  if p_changed_plans is null or jsonb_typeof(p_changed_plans) <> 'array' then
  ...
  -- Keep the validated plan identity stable until the heartbeat update. This
  -- also closes the absent-row race for no-change validation.
  ...
  mismatches as (
    ...
    where (
        ...
        and (
          ...
          or to_jsonb(current_plan.planned_hours) is distinct from expected.expected_planned_hours
        )
      )
      or (
  ...
  set health_status = 'healthy',
      last_outcome = case when changed_plan_count > 0 then 'published' else 'no_changes' end,
      reason = p_publish_reason,
      last_published_at = case
        when changed_plan_count > 0 then p_published_at
