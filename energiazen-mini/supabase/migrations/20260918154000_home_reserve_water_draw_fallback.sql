-- Keep the last reliable V2 reserve available to the Home presentation while
-- the latest run is temporarily unavailable because a water draw is unresolved.
-- The app applies the age/reason policy; this RPC only exposes the latest good
-- row together with the latest run status.
...
as $function$
  ...
  cross join latest
  where good.run_at >= now() - interval '24 hours';
$function$;
