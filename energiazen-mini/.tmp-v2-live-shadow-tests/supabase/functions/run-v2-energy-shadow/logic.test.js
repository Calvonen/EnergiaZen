...
exports.runLiveV2EnergyShadowUnitTests = runLiveV2EnergyShadowUnitTests;
...
function reading(timestamp, topTemp, bottomTemp, inletTemp, heating) {
    return {
...
function runLiveV2EnergyShadowUnitTests() {
    const capacityBaseline = (0, logic_1.deriveUsableReadingInletBaselineC)([
    ...
    assertClose(capacityBaseline, 12, "capacity baseline excludes the same unusable readings as the reserve replay");
    const laggingSensors = (0, logic_1.runLiveReserveShadow)({
        maxTankTemperatureC,
    ...
    assert((laggingSensors.remainingEnergyKwh ?? 0) > (laggingSensors.observedEnergyKwh ?? 0), "known relay-on heater input survives sensor lag below the thermostat guard");
    ...
    assertClose(laggingSensors.conservativeEnergyKwh, (laggingSensors.remainingEnergyKwh ?? 0) - 0.25, "reserve safety subtracts balance uncertainty, not the sensor gap");
    assert(laggingSensors.safetyEnergyKwh === 3 && laggingSensors.targetEnergyKwh === 6, "live shadow uses the shared V2 reserve thresholds");
    assertClose(laggingSensors.heaterCreditGuardTopTempC, 63, "65 C max setting creates a 63 C heater credit guard");
    const warmerSensorsWithoutInput = (0, logic_1.runLiveReserveShadow)({
        maxTankTemperatureC,
    ...
    assert((warmerSensorsWithoutInput.remainingEnergyKwh ?? 0) < (warmerSensorsWithoutInput.observedEnergyKwh ?? 0), "warmer sensors cannot mint physical energy without an explicit source");
    assert((warmerSensorsWithoutInput.sensorGapKwh ?? 0) < 0, "model below observation is retained as a signed diagnostic gap");
    const thermostatTransition = (0, logic_1.runLiveReserveShadow)({
        maxTankTemperatureC,
    ...
    assertClose(thermostatTransition.heaterDeliveryUncertaintyKwh, 0.05, "one uncertain minute at 3 kW contributes 0.05 kWh delivery uncertainty");
    assertClose(thermostatTransition.balanceUncertaintyKwh, 0.3, "heater delivery uncertainty is added to the 0.25 kWh baseline");
    const unresolvedDraw = (0, logic_1.runLiveReserveShadow)({
        maxTankTemperatureC,
    ...
    const stabilizedUnlabeledReadings = [];
    ...
    const stabilizedUnlabeledDraw = (0, logic_1.runLiveReserveShadow)({
        maxTankTemperatureC,
    ...
    assert(stabilizedUnlabeledDraw.available, "unlabeled draw becomes available again after inlet recovery and 15 quiet minutes");
    assert(!stabilizedUnlabeledDraw.unresolvedDrawDetected, "completed stabilized draw is not kept unresolved for the whole replay window");
    assert(Math.abs((stabilizedUnlabeledDraw.sensorGapKwh ?? 99)) < 0.2, "post-draw re-anchor keeps the forward physical balance close to the stabilized observation");
    const acceptedDraw = {
    ...
    const resolvedDraw = (0, logic_1.runLiveReserveShadow)({
        maxTankTemperatureC,
    ...
    assert((resolvedDraw.remainingEnergyKwh ?? Infinity) < (resolvedDraw.observedEnergyKwh ?? -Infinity), "later warm sensor data cannot add the accepted water-draw removal back");
    assert(resolvedDraw.v1NeedsEnergyRecovery === false, "zero V1 target hours maps to no recovery need");
    const telemetryGap = (0, logic_1.runLiveReserveShadow)({
        maxTankTemperatureC,
    ...
    const staleLatest = (0, logic_1.runLiveReserveShadow)({
        maxTankTemperatureC,
    ...
    const unavailableWithDerivedThresholds = (0, logic_1.applyReserveThresholds)(staleLatest, 5.4, 17.1);
    assert(!unavailableWithDerivedThresholds.available, "thresholds do not mask replay unavailability");
    assert(unavailableWithDerivedThresholds.reason === "latest_tank_reading_stale", "derived thresholds preserve the replay failure reason");
    assertClose(unavailableWithDerivedThresholds.safetyEnergyKwh, 5.4, "unavailable runs persist the percentage-derived safety threshold");
    assertClose(unavailableWithDerivedThresholds.targetEnergyKwh, 17.1, "unavailable runs persist the percentage-derived target threshold");
}
